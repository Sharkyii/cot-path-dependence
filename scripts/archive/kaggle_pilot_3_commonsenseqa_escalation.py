# ============================================================
# NOTEBOOK 3 of 3 -- COMMONSENSEQA ESCALATION (N=100, problem-disjoint)
#
# Purpose: CommonsenseQA came back INCONCLUSIVE in the original pilot --
# delta@0.60 = +0.125, 95% CI [+0.025, +0.226], straddling the 0.10
# headroom threshold. PILOT_PREREGISTRATION.md's pre-committed escalation
# rule: re-run at N=100 on problems DISJOINT from the original 40, then
# re-apply the identical three-way PASS/FAIL/INCONCLUSIVE rule.
#
# PROBLEM_START/END below select indices [40, 140) -- i.e. the 100
# problems immediately after the original 40 in the same deterministic
# dataset ordering, so this sample cannot overlap the Stage-1 sample.
#
# Per the pre-registration: this is the ONLY escalation. If this run is
# ALSO inconclusive, the pre-committed action is to DROP the domain --
# do not escalate a second time (that would be sampling until
# significance, exactly what pre-registration exists to prevent).
# ============================================================
...
N_PROBLEMS = 140        # fallback only; PROBLEM_START/END below select the real slice
...
PROBLEM_START = 40     # skip the original Stage-1 sample (indices 0-39)
PROBLEM_END = 140      # -> 100 NEW problems, disjoint from the original 40
...
DOMAINS = ["CommonsenseQA"]   # fixed -- this notebook is the escalation run only
