# ============================================================
# CANDIDATE B FULL PILOT -- difficulty stratum 2 of 3
# Paste as ONE Kaggle cell. Requires: Settings -> Accelerator -> GPU T4 x2
#                                     Settings -> Internet -> ON (or mount model)
#
# Implements CANDIDATE_B_PREREGISTRATION.md's design for ONE MATH-500
# difficulty stratum (level 2): 6 problems x 5 trajectories,
# 8 matched pairs branched at N_BRANCHES_PER_PAIR=40, confidence- AND
# problem_id-stratified sampling (fixes the problem-level imbalance found
# in the micro-pilot -- see DESIGN_CANDIDATE_B.md SS4.6/4.8).
#
# This notebook is FULLY SELF-CONTAINED for its stratum -- no cross-notebook
# problem_id coordination needed (unlike the micro-pilot's PROBLEM_START/END
# split), since each of the 3 strata (level 1, 2, 3) is run independently
# and results are combined afterward by difficulty level, not by raw index.
# Run all 3 (level 1/2/3 scripts) in separate Kaggle sessions, then combine
# the 3 pair_results.json outputs for the final Fisher's-method verdict.
#
...
#   - stratified_sample_pairs now stratifies by BOTH confidence AND
#     problem_id (fix for the imbalance found in micro-pilot run #2, where
#     one problem's abundance of matched pairs could still starve other
#     problems within a confidence bucket even with confidence
#     stratification alone working correctly).
# ============================================================
...
# ------------------------------------------------------------
# FULL PILOT SIZE -- CANDIDATE_B_PREREGISTRATION.md SS1-3, this stratum only
# ------------------------------------------------------------
DIFFICULTY_LEVEL = "2"    # MATH-500's own level field -- this notebook's stratum
N_PROBLEMS_THIS_LEVEL = 6                  # SS3: 6 problems per difficulty level, 18 total across 3 notebooks
K_TRAJECTORIES_PER_PROBLEM = 5
N_BRANCHES_PER_PAIR = 40                   # SS4.9 power-corrected value (was 15 in the micro-pilot --
                                            # power analysis showed 15 gives only 3-9% power to detect a
                                            # plausible small effect regardless of pair count; 40 gets to ~89%)
MAX_PAIRS_TO_BRANCH = 8                    # SS3: 8 pairs per stratum (24 total across all 3 notebooks)
...
POSITION_TOL = 0.15
MIN_PARSE_RATE = 0.80
OUTPUT_DIR = f"candidate_b_fullpilot_level{DIFFICULTY_LEVEL}"
...
def stratified_sample_pairs(pairs, budget, confidence_bounds=(0.0, 0.5, 0.85, 1.01), seed=0):
    """Sample across confidence strata, AND across problem_id within each
    confidence stratum, rather than taking the first N found.

    Confidence stratification fixes a bug found in the micro-pilot's own
    first run: once a trajectory converges, essentially every later step
    force-extracts to the SAME answer at HIGH confidence, so naive
    pairs[:budget] draws almost entirely from that regime.

    Problem_id stratification (added for the full pilot -- see
    DESIGN_CANDIDATE_B.md SS4.6/4.8) fixes a SECOND, independent bug found
    in micro-pilot run #2: confidence stratification alone still let one
    problem's abundance of matched pairs dominate a bucket's budget when
    that problem simply had more matched pairs than its neighbors --
    notebook 1/3's entire 8-pair budget landed on only 2 of its 3 problems
    even with confidence strata working exactly as designed.
    """
...
# ------------------------------------------------------------
# DATA -- this stratum's 6 problems only (MATH-500 level == DIFFICULTY_LEVEL)
# ------------------------------------------------------------
math_ds = load_dataset("HuggingFaceH4/MATH-500", split="test")
math_ds = math_ds.filter(lambda r: str(r["level"]) == DIFFICULTY_LEVEL)
if len(math_ds) < N_PROBLEMS_THIS_LEVEL:
    raise SystemExit(f"Only {len(math_ds)} MATH-500 problems at level {DIFFICULTY_LEVEL}, "
                      f"need {N_PROBLEMS_THIS_LEVEL} -- reduce N_PROBLEMS_THIS_LEVEL")
problems = math_ds.select(range(N_PROBLEMS_THIS_LEVEL))
print(f"Loaded {len(problems)} MATH-500 level-{DIFFICULTY_LEVEL} problems for the full pilot\n")
...
# problem_id is prefixed with the difficulty level (e.g. "d2_prob0")
# so results from all 3 stratum notebooks can be safely concatenated later
# without any risk of id collision -- each stratum is fully self-contained,
# no PROBLEM_START/END global-offset coordination needed (unlike the
# micro-pilot's arbitrary problem-range split).
trace_steps_cache = {}

print("=" * 78)
print(f"PHASE 1: diverse trajectories + phi(p_t) at every step, level {DIFFICULTY_LEVEL} problems")
print("=" * 78)

for pi, item in enumerate(problems):
    problem_id = f"d{DIFFICULTY_LEVEL}_prob{pi}"
    base_prompt = make_prompt(item["problem"])
    print(f"\n--- {problem_id} ---")
    for ti in range(K_TRAJECTORIES_PER_PROBLEM):
        traj_id = f"traj_{ti}"  # assigned BEFORE anything that can raise
        try:
            t0 = time.time()
...
# ------------------------------------------------------------
# PHASE 3: natural branching + PDI on this stratum's pair budget
# ------------------------------------------------------------
print("\n" + "=" * 78)
print(f"PHASE 3: natural branching (N={N_BRANCHES_PER_PAIR}/prefix, NO forced suffix), "
      f"capped at {MAX_PAIRS_TO_BRANCH} pairs (this stratum)")
print("=" * 78)
...
for level in ("L1", "L2", "L3"):
    if budget <= 0:
        break
    level_seed = {"L1": 1, "L2": 2, "L3": 3}[level]  # NOT hash(level) -- non-deterministic per-process
    sampled_pairs = stratified_sample_pairs(pairs_by_level[level], budget, seed=level_seed)
    for a, b in sampled_pairs:
        try:
            problem_row = problems[int(a.problem_id.split("_prob")[1])]  # local index, this stratum only
...
# ------------------------------------------------------------
print("\n\n" + "#" * 78)
print(f"FULL PILOT REPORT -- DIFFICULTY LEVEL {DIFFICULTY_LEVEL}")
print("#" * 78)
...
if pair_results:
    ...
    if len(pair_results) >= 2:
        ...
        print(f"\n   Combined (Fisher's method, {len(pair_results)} independent pairs, THIS STRATUM ONLY): p={combined_p:.4f}")
...
print(f"\nThis is difficulty stratum {DIFFICULTY_LEVEL} of 3 (CANDIDATE_B_PREREGISTRATION.md).")
print("Combine this stratum's pair_results.json with the other 2 strata's for the")
print("overall Fisher's-method verdict across all 24 pairs -- see SS5 of the pre-registration.")
