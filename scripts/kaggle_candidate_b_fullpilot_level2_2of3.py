# ============================================================
# CANDIDATE B FULL PILOT -- difficulty stratum 2 of 3, sub-split 2of3 of 3
...
# MATH-500 difficulty stratum (level 2, problems 2-3 of 6).
...
#
# problem_id is "d2_prob{pi}" with pi in [2,4) -- the SAME
# global numbering the undivided level-2 script would have used, so this
...
# ------------------------------------------------------------
DIFFICULTY_LEVEL = "2"    # MATH-500's own level field -- this stratum
N_PROBLEMS_THIS_LEVEL_TOTAL = 6            # SS3: 6 problems per difficulty level, ACROSS all 3 sub-notebooks
SUB_START = 2                    # this sub-notebook's problem slice within the stratum
SUB_END = 4                        # exclusive
...
# ------------------------------------------------------------
# DATA -- this stratum's problems 2-3 of 6 (MATH-500 level == DIFFICULTY_LEVEL)
# ------------------------------------------------------------
...
for _local_pi, item in enumerate(problems):
    ...
    # undivided level-2 script (or the other 2 sub-notebooks of this same
...
for level in ("L1", "L2", "L3"):
    ...
    for a, b in sampled_pairs:
        try:
            ...
            # ends. Missing from the first 2 completed sub-notebooks
            # (level1_1of3, level3_1of3) -- added once real significant
            # results started appearing and the raw breakdown was wanted.
